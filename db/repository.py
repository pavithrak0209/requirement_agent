...
from core.requirements_pod.db.models import SourceFile, Task
from core.requirements_pod.schemas.task import TaskUpdate
from core.requirements_pod.schemas.file import FileStatus
...
def get_story_task_ids_for_file(db: Session, file_id: str) -> tuple[list[str], list[str]]:
    """Return (story_task_ids, all_task_ids) for non-deleted tasks in a file."""
    tasks = db.execute(
        select(Task).where(Task.source_file_id == file_id, Task.status != "deleted")
    ).scalars().all()
    story_ids = [t.task_id for t in tasks if t.task_type == "story"]
    all_ids   = [t.task_id for t in tasks]
    return story_ids, all_ids
